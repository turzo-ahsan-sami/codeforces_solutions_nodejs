...
const has_ab_after_ba = (inp) => {
    let doesContain_AB = inp.includes('AB')
    inp = inp.replace('AB')
    let doesContain_BA = inp.includes('BA')

    return ( doesContain_AB && doesContain_BA)
}

const has_ba_after_ab = (inp) => {
    let doesContain_BA = inp.includes('BA')
    inp = inp.replace('BA')
    let doesContain_AB = inp.includes('AB')
...
const tempPrintFunc = (inp) => {
    ...
    return ( has_ab_after_ba(inp) || has_ba_after_ab(inp) ) ? 'YES' : 'NO'
}

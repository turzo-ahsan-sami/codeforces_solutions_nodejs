...
const calculateCapacity = (outs, ins) => {
    let max_person_inside = []
    max_person_inside[0] = ins[0]

    for (let i = 1; i < ins.length; i++) {
        max_person_inside[i] = max_person_inside[i - 1] + (ins[i] - outs[i])
    }

    return Math.max.apply(Math, max_person_inside)
}
...
function main() {
    ...
    let i = 0
    while (stops--) {
        let ins_outs = readline().split(' ').map(Number)
        outs[i] = ins_outs[0]
        ins[i] = ins_outs[1]
        i++
    }

    console.log(calculateCapacity(outs, ins))
}
//*/
